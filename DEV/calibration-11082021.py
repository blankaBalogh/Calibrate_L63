...
#nn_L63.model.load_weights('weights/weights'+nn_L63.suffix+'-e10.h5')
...
if new_gp :
    ...
    if exp=='2d' :
        ic[:,:3], ic[:,4:] = X0, Thetas
        ic[:,3] = np.repeat(10., Thetas.shape[0])
    else :
        ic[:,:3], ic[:,5] = X0, Thetas[...,0]
        ic[:,3] = np.repeat(10., Thetas.shape[0])
        ic[:,4] = np.repeat(28., Thetas.shape[0])
...
alpha = 0.5
mean_err_std, std_err_std = np.mean(err_std), np.std(err_std)
mean_err_mean, std_err_mean = np.mean(err_mean), np.std(err_mean)
err_std = (err_std-mean_err_std)/std_err_std
err_mean = (err_mean-mean_err_mean)/std_err_mean
...
if exp=='2d' :
    theta_to_update = [28.5, 2.5] 
else :
    theta_to_update = [2.5]         # optimization starting point (or ic)
